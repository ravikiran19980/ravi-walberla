...
void loadFromConfig(Parameters& params, const Config::BlockHandle& cfg)
{
   ...
   params.normal = cfg.getParameter<Vec3>("normal", Vec3(1.0_r, 1.0_r, 1.0_r) );
   WALBERLA_LOG_INFO_ON_ROOT("normal: " << params.normal);
   
   params.spacing = cfg.getParameter<real_t>("spacing", 1.0_r );
   WALBERLA_LOG_INFO_ON_ROOT("spacing: " << params.spacing);
   
   params.shift = cfg.getParameter<Vec3>("shift", Vec3(0.1_r, 0.1_r, 0.1_r) );
   WALBERLA_LOG_INFO_ON_ROOT("shift: " << params.shift);
   
   params.radius = cfg.getParameter<real_t>("radius", 0.5_r );
   ...
   params.dt = cfg.getParameter<real_t>("dt", 0.01_r );
   ...
   params.baseWeight = cfg.getParameter<real_t>("baseWeight", 10.0_r );
   WALBERLA_LOG_INFO_ON_ROOT("baseWeight: " << params.baseWeight);
   
   params.metisipc2redist = cfg.getParameter<real_t>("metisipc2redist", 1000.0_r );
