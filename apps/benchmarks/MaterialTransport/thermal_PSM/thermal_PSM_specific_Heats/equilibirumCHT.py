...
class DiscreteThermalMaxwellianCHT(GenericDiscreteEquilibrium):
    ...
    def __init__(self, stencil, compressible=True,
        ...
        pdfs = discrete_thermal_equilibrium_cht(stencil, rho_Cp_T=self._rho_Cp_T, u=u,
                                                ...
                                                compressible=compressible,substitutions = self._substitutions, Cp_ref = self.Cp_ref, temperature = temperature,Cp_f=Cp_f, rho_f=rho_f)
...
def discrete_thermal_equilibrium_cht(stencil, rho_Cp_T=sp.Symbol("rho_Cp_T"), u=sp.symbols("u_:3"), order=2,
                                     c_s_sq=sp.Symbol("c_s") ** 2, compressible=True,substitutions=None,Cp_ref=None, temperature=sp.Symbol("T"),Cp_f=sp.Symbol("Cp_f"), rho_f=sp.Symbol("rho_f")):
    """
    ...
    u = u[:stencil.D]
    res = [thermal_equilibrium_cht(e_q, u, rho_Cp_T, w_q, order, c_s_sq, compressible,substitutions, Cp_ref, temperature, Cp_f, rho_f) for w_q, e_q in zip(weights, stencil)]
    return tuple(res)

def thermal_equilibrium_cht(v=sp.symbols("v_:3"), u=sp.symbols("u_:3"), rho_Cp_T=sp.Symbol("rho_Cp_T"), weight=sp.Symbol("w"),
                            order=2, c_s_sq=sp.Symbol("c_s") ** 2, compressible=True,substitutions=None, Cp_ref=None, temperature=sp.Symbol("T"),Cp_f=sp.Symbol("Cp_f"), rho_f=sp.Symbol("rho_f")):
    """
    ...
    fq = Cp_ref / (rho_f * Cp_f) + e_times_u / c_s_sq
    ...
    if v == (0, 0) or v == (0, 0, 0):
        result = rho_Cp_T - Cp_ref * rho_Cp_T/(rho_f*Cp_f) + fq * weight * rho_Cp_T
