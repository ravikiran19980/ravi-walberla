...
public:
    ...
    void operator()(IBlock * block)
    {
        ...
        WALBERLA_FOR_ALL_CELLS_XYZ(src, {
                    ...
                    // collide
                    const real_t vel_sqr = u.sqrLength() * 1.5_r;
                    for( auto d = LM::Stencil::begin(); d != LM::Stencil::end(); ++d )
                    {
                        const real_t pdf = src->get(x - d.cx(), y - d.cy(), z - d.cz(), d.toIdx());
                        const real_t vel = real_c(d.cx())*u[0] + real_c(d.cy())*u[1] + real_c(d.cz())*u[2];
                        dst->get(x, y, z, d.toIdx()) = ( 1.0_r - omega_ ) * pdf +
                                                       omega_ * LM::w[ d.toIdx() ] * ( rho - vel_sqr + 3.0_r*vel + 4.5_r*vel*vel );
                    }
                });
...
public:
    ...
    void operator()(IBlock * block)
    {
        ...
        real_t omega_trm_ = 1_r - omega_;
        real_t omega_w0_  = 3_r * ( 1_r / 3_r ) * omega_;
        real_t omega_w1_  = 3_r * ( 1_r / 18_r ) * omega_;
        real_t omega_w2_  = 3_r * ( 1_r / 36_r ) * omega_;
