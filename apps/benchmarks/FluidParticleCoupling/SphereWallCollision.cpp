...
public:
   ...
   Type * operator()( IBlock* const block, const StructuredBlockStorage* const storage ) const
   {
      ...
      Type * handling = new Type( "moving obstacle boundary handling", flagField, fluid,
                                  ...
                                  SimplePressure_T( "SimplePressure", SimplePressure_Flag, pdfField, 1_r ) );
...
public:
   SpherePropertyLogger( const shared_ptr< ParticleAccessor_T > & ac, walberla::id_t sphereUid,
      ...
      gap_( 0_r ), settlingVelocity_( 0_r )
   {
      if ( fileIO_ )
      {
...
{
   real_t evalRadius = 0.5_r * diameter + surfaceDistance;
   std::vector< Vector3<real_t> > evaluationPositions;
   evaluationPositions.push_back(spherePosition + Vector3<real_t>(0_r, 0_r, evalRadius));
   evaluationPositions.push_back(spherePosition + Vector3<real_t>(evalRadius / real_c(sqrt(2_r)), 0_r, real_c(evalRadius / sqrt(2_r))));
   evaluationPositions.push_back(spherePosition + Vector3<real_t>(evalRadius, 0_r, 0_r));
   evaluationPositions.push_back(spherePosition + Vector3<real_t>(evalRadius / real_c(sqrt(2_r)), 0_r, -evalRadius / real_c(sqrt(2_r))));
   evaluationPositions.push_back(spherePosition + Vector3<real_t>(0_r, 0_r, -evalRadius));

   std::vector<real_t> densityAtPos(evaluationPositions.size(), 0_r);
...
int main( int argc, char **argv )
{
   ...
   const real_t ug_SI = std::sqrt((densityRatio-1_r) * gravitationalAcceleration_SI * diameter_SI);
   ...
   const real_t St = densityRatio * uIn * diameter / ( 9_r * viscosity );
   ...
   const real_t responseTime = densityRatio * diameter * diameter / ( 18_r * viscosity );
   const real_t accelerationFactor = 1_r / (0.1_r * responseTime);
   ...
   const real_t uInCrit = 9_r * StCrit * viscosity / ( densityRatio * diameter);
   ...
   real_t initialSpherePosition = real_c(domainSize[2]) - 1.5_r * diameter;
   ...
   if( randomizeInitialSpherePosition )
   {
      ...
      real_t initialPositionOffset = 0.5_r*math::realRandom<real_t>(-1_r, 1_r, g1);
   ...
   WALBERLA_LOG_INFO_ON_ROOT(" - fluid: density = " << densityFluid << ", relaxation time (tau) = " << 1_r / omega << ", omega = " << omega << ", kin. visc = " << viscosity );
   ...
   if( useLubricationCorrection )
   {
      ...
      WALBERLA_LOG_INFO_ON_ROOT("  - lubrication correction minimal gap size non dim = " << lubricationMinimalGapSizeNonDim  << " ( = " << lubricationMinimalGapSizeNonDim * diameter * 0.5_r << " cells )");
   }
   ...
   Vector3<real_t> initialPosition( 0.5_r * real_c(domainSize[0]), 0.5_r * real_c(domainSize[1]), initialSpherePosition );
   ...
   } else
   {
      ...
      if (rpdDomain->isContainedInProcessSubdomain( uint_c(mpi::MPIManager::instance()->rank()), initialPosition ))
      {
         ...
         p.setType(0);
         if( initializeSphereVelocity ) p.setLinearVelocity(Vector3<real_t>(0_r, 0_r, -0.1_r * uIn));
         sphereUid = p.getUid();
      }
   }
   ...
   linearCollisionResponse.setStiffnessAndDamping(0,0,restitutionCoeff,collisionTime,0_r,Mij); // no response in tangential direction
   ...
   lbm_mesapd_coupling::LubricationCorrectionKernel lubricationCorrectionKernel(viscosity, [lubricationMinimalGapSizeNonDim](real_t r){ return lubricationMinimalGapSizeNonDim * r;}, lubricationCutOffDistance, 0_r, 0_r ); // no tangential components needed
   ...
   // create the timeloop
   const uint_t timesteps = uint_c( 3_r * initialPosition[2] / uIn);
   ...
   if( vtkIOFreq != uint_t{0} )
   {
      ...
      AABB sliceAABB( 0_r, real_c(domainSize[1])*0.5_r-1_r, 0_r,
                      real_c(domainSize[0]), real_c(domainSize[1])*0.5_r+1_r, real_c(domainSize[2]) );
   ...
   real_t curVel(0_r);
   real_t oldVel(0_r);
   real_t maxSettlingVel(0_r);
   std::vector<real_t> maxSettlingVelBetweenBounces;

   real_t minGapSize(0_r);
   real_t maxGapSize(0_r);
   ...
   AABB boxAABB( initialPosition[0] - diameter, initialPosition[1] - diameter, -1_r,
                 initialPosition[0] + diameter, initialPosition[1] + diameter, 2_r * diameter);
   ...
   for (uint_t i = 0; i < timesteps; ++i )
   {
      ...
      for(auto subCycle = uint_t{0}; subCycle < numRPDSubCycles; ++subCycle )
      {
         ...
         if( useVelocityVerlet )
         {
            ...
            if(artificiallyAccelerateSphere)
            {
               ...
               if(idx != accessor->getInvalidIdx())
               {
                  ...
                  accessor->setOldForce(idx, Vector3<real_t>(0_r));
               }
            }
         ...
         if( artificiallyAccelerateSphere)
         {
            ...
            real_t newSphereVel = uIn * (std::exp(-accelerationFactor * static_cast< real_t >(i) ) - 1_r);
            ps->forEachParticle(useOpenMP, sphereSelector, *accessor, [newSphereVel](const size_t idx, ParticleAccessor_T& ac){ ac.setLinearVelocity(idx, Vector3<real_t>(0_r, 0_r, newSphereVel));}, *accessor);
         }
      ...
      // detect a bounce
      if( oldVel < 0_r && curVel > 0_r && logger.getGapSize() < 1_r)
      {

         ++numBounces;
         WALBERLA_LOG_INFO_ON_ROOT("Detected bounce number " << numBounces);
         WALBERLA_LOG_INFO_ON_ROOT("Max settling velocity " << maxSettlingVel << " -> St = "  << densityRatio * std::abs(maxSettlingVel) * diameter / ( 9_r * viscosity ) );

         if(numBounces == uint_t{1})
         {
            actualSt = densityRatio * std::abs(maxSettlingVel) * diameter / ( 9_r * viscosity );
      ...
      if( tImpact == uint_t{0} && numBounces == uint_t{1} && logger.getGapSize() > 0_r )
      {
         tImpact = i;
      ...
      if( numBounces > impactTimes.size() && logger.getGapSize() > 0_r)
      {
      ...
      if( logger.getGapSize() < 1_r * diameter ) {
      ...
      if(vtkOutputOnCollision && logger.getGapSize() < 0_r)
      {
   ...
   {
      WALBERLA_LOG_INFO_ON_ROOT(" - vel = " << vel << " -> St = "  << densityRatio * std::abs(vel) * diameter / ( 9_r * viscosity ) << ", Re = " << std::abs(vel) * diameter / viscosity );
   }
   ...
   WALBERLA_ROOT_SECTION()
   {
      ...
      for( auto vel : maxSettlingVelBetweenBounces)
      {
        file << " - " << bounce++ << ": vel = " << vel << " -> St = "  << densityRatio * std::abs(vel) * diameter / ( 9_r * viscosity ) << ", Re = " << std::abs(vel) * diameter / viscosity << "\n";
      }
      file << "maximal overlap = " << std::abs(minGapSize) << " (" << std::abs(minGapSize)/diameter * 100_r << "%)\n";
