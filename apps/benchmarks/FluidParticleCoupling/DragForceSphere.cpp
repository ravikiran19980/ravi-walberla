...
int main( int argc, char **argv )
{
   ...
   real_t externalForcing = 1e-8_r;
   uint_t length  = uint_c( 40 );

   MEMVariant method = MEMVariant::CLI;
   real_t bulkViscRateFactor = 1_r; // ratio between bulk and shear viscosity related relaxation factors, 1 = TRT, > 1 for stability with MRT
   ...
   const real_t convergenceLimit = 0.1_r * setup.extForce;           // tolerance for relative change in drag force
   ...
   WALBERLA_LOG_INFO_ON_ROOT("diameter = " << 2_r * setup.radius);
   ...
   WALBERLA_LOG_INFO_ON_ROOT("Re = " << forceEval->getAverageVel() * setup.radius * 2_r / setup.visc);
