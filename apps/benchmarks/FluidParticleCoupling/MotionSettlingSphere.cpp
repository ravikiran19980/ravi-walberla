...
public:
   ...
   void operator()(const uint_t timestep)
   {
      Vector3<real_t> particleTransVel( 0_r );
      Vector3<real_t> particleAngularVel( 0_r );
      Vector3<real_t> particlePos( 0_r );

      Vector3<real_t> particleForce( 0_r );
      Vector3<real_t> particleTorque( 0_r );
...
int main( int argc, char **argv )
{
   ...
   real_t viscosity = 0.01_r; // used in diffusive scaling
   real_t uIn = 0.02_r; // used for acoustic scaling
   ...
   real_t vtkWriteSpacingNonDim = 3_r; // write every 3 non-dim timesteps
   ...
   uint_t xlength = uint_c( diameter * 5.34_r );
   ...
   BlockDataID pdfFieldID = lbm::addPdfFieldToStorage< LatticeModel_T >( blocks, "pdf field", latticeModel, initFluidVelocity ? uInfty : Vector3<real_t>(0), 1_r, uint_t{1}, field::fzyx );
   ...
   timeloopInit.addFuncAfterTimeStep( RemainingTimeLogger( timeloopInit.getNrOfTimeSteps(), 30_r ), "Remaining Time Logger" );
   ...
   while (true) {
      ...
      gravity = forceEval.getDragForce() / ( (densityRatio - 1_r ) * diameter * diameter * diameter * math::pi / 6_r );
      ...
      defaultOmegaBulk = lbm_mesapd_coupling::omegaBulkFromOmega(omega, 1_r);
   ...
   real_t dtSim = (averageForceTorqueOverTwoTimeStepsMode == 2) ? 2_r : 1_r;
   ...
   timeloop.addFuncBeforeTimeStep( RemainingTimeLogger( timeloop.getNrOfTimeSteps(), 30_r ), "Remaining Time Logger" );
   ...
   for (uint_t i = 0; i < timesteps; ++i )
   {
      ...
      if(movingSpherePropertyEvaluator.getParticleHeight() > real_c(zlength) - 3_r * diameter)
      {
