...
void createPlaneSetup(const shared_ptr<mesa_pd::data::ParticleStorage> & ps, const shared_ptr<mesa_pd::data::ShapeStorage> & ss, const math::AABB & simulationDomain, real_t velocity = 0_r)
{
   ...
   p2.setType(0);
   p2.setLinearVelocity(Vector3<real_t>(0_r,0_r,velocity));
   ...
   p3.setType(0);
   p3.setLinearVelocity(Vector3<real_t>(0_r,0_r,velocity));
   ...
   p4.setType(0);
   p4.setLinearVelocity(Vector3<real_t>(0_r,0_r,velocity));
   ...
   p5.setType(0);
   p5.setLinearVelocity(Vector3<real_t>(0_r,0_r,velocity));
...
int main( int argc, char **argv )
{
   ...
   real_t initialSpherePosition = 0.5_r; // in ratio to domain height

   // simulation setup
   real_t velocity = 0.02_r;
   real_t Re = 164_r;
   real_t diameter = 20_r;
   real_t numberOfTimeStepsNonDim = 20_r;
   real_t domainWidthNonDim = 4_r;
   real_t domainHeightNonDim = 8_r;
   real_t accelerationFactor = 3_r;
   ...
   const real_t relaxationTime = 1_r / omega;
   ...
   Vector3<real_t> initialPosition( 0.5_r * real_c(domainSize[0]), 0.5_r * real_c(domainSize[1]), initialSpherePosition * real_c(domainSize[2]));
   ...
   if (rpdDomain->isContainedInProcessSubdomain( uint_c(mpi::MPIManager::instance()->rank()), initialPosition ))
   {
      ...
      if(!useGalileanInvariantSetup && !artificiallyAccelerateSphere) p.setLinearVelocity(Vector3<real_t>(0_r,0_r,velocity));
      sphereUid = p.getUid();
   }
   ...
   BlockDataID pdfFieldID = lbm::addPdfFieldToStorage< LatticeModel_T >( blocks, "pdf field (fzyx)", latticeModel,
                                                                         useGalileanInvariantSetup ? Vector3<real_t>(0_r, 0_r, -velocity) : Vector3< real_t >( 0_r),
                                                                         1_r,
                                                                         uint_t{1}, field::fzyx );
   // add flag field
   ...
   std::function<void(void)> syncCall = [ps,rpdDomain, adaptionLayerSize ](){
      const real_t overlap = 1.5_r + std::max(0_r, adaptionLayerSize - 2_r);
   ...
   real_t dtSim = 1_r;
   if (forceTorqueAveragingType == "twoLBM") dtSim = 2_r;
   ...
   SpherePropertyLogger<ParticleAccessor_T > logger( accessor, sphereUid, loggingFileName, fileIO, diameter, velocity,  3_r * math::pi * diameter * viscosity * velocity);
   ...
   for (uint_t i = 0; i < uint_c(real_c(timesteps)/dtSim); ++i )
   {
      ...
      auto movingProbePosition = Vector3<real_t>(real_c(domainSize[0])*0.75_r, real_c(domainSize[1])*0.5_r, logger.getPosition());
      if(logDensity)
      {
         density1 = getDensityAtPosition<DensityInterpolator_T >(blocks, densityInterpolatorID, Vector3<real_t>(real_c(domainSize[0])*0.25_r, real_c(domainSize[1])*0.5_r, real_c(domainSize[2])*0.5_r));
      ...
      Vector3<real_t> velocityAtProbePos(0_r);
      if(logFluidVelocity)
      {
      ...
      if(!fixedSphere)
      {
         ...
         {
            real_t newSphereVel = - velocity * (std::exp(- static_cast< real_t >(i) * dtSim * accelerationFactor/ tref ) - 1_r);
