...
inline real_t fittedLBMWeightEvaluationFunction(const BlockInfo& blockInfo)
{
   ...
   real_t weight = real_t(7.597476065046571e-06) * real_c(Ce) + real_t(8.95723566283202e-05) * real_c(F) + -0.1526111388616016_r;
   return std::max(weight,0_r);
}
inline real_t fittedBHWeightEvaluationFunction(const BlockInfo& blockInfo)
{
   uint_t Ce = blockInfo.numberOfCells;
   uint_t NB = blockInfo.numberOfNearBoundaryCells;
   real_t weight = real_t(1.3067711379655123e-07) * real_c(Ce) + 0.0007289549127205142_r * real_c(NB) + -0.1575698071795788_r;
   return std::max(weight,0_r);
}
inline real_t fittedRPDWeightEvaluationFunction(const BlockInfo& blockInfo)
{
   ...
   real_t cPl    = 0.00040932622363097144_r;
   real_t cPg    = 0.0007268941363125683_r;
...
inline real_t fittedCoup1WeightEvaluationFunction(const BlockInfo& blockInfo)
{
   ...
   uint_t Pl = blockInfo.numberOfLocalParticles;
   uint_t Pg = blockInfo.numberOfGhostParticles;
   real_t weight = real_t(5.610203409278647e-06) * real_c(Ce) + real_t(-7.257635845636656e-07) * real_c(F) + 0.02049703546054693_r * real_c(Pl) + 0.04248208493809902_r * real_c(Pg) + -0.26609470510074784_r;
   return std::max(weight,0_r);
}
inline real_t fittedCoup2WeightEvaluationFunction(const BlockInfo& blockInfo)
{
   ...
   uint_t Pl = blockInfo.numberOfLocalParticles;
   uint_t Pg = blockInfo.numberOfGhostParticles;
   real_t weight = real_t(7.198479654682179e-06) * real_c(Ce) + real_t(1.178247475854302e-06) * real_c(F) + -0.0026401549115124628_r * real_c(Pl) + 0.008459646786179298_r * real_c(Pg) + -0.001077320113275954_r;
   return std::max(weight,0_r);
}
