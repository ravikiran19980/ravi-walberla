...
int main( int argc, char ** argv )
{
   ...
   const real_t spacing = mainConf.getParameter<real_t>("spacing", 1.0_r );
   WALBERLA_LOG_INFO_ON_ROOT("spacing: " << spacing);

   const real_t radius = mainConf.getParameter<real_t>("radius", 0.5_r );
   WALBERLA_LOG_INFO_ON_ROOT("radius: " << radius);

   const real_t vMax = mainConf.getParameter<real_t>("vMax", 0.5_r );
   WALBERLA_LOG_INFO_ON_ROOT("vMax: " << vMax);

   const real_t rotationSpeed = mainConf.getParameter<real_t>("rotationSpeed", 0.5_r );
   WALBERLA_LOG_INFO_ON_ROOT("rotationSpeed: " << rotationSpeed);

   const Vec3 gravity = mainConf.getParameter<Vec3>("gravity", Vec3(0, 0, 9.81_r));
   WALBERLA_LOG_INFO_ON_ROOT("rotationSpeed: " << rotationSpeed);
   ...
   for (auto& iBlk : *forest)
   {
      for (auto pt : grid_generator::SCGrid(iBlk.getAABB(), Vector3<real_t>(spacing, spacing, spacing) * real_c(0.5), spacing))
      {
         ...
         auto maxDist = real_t(simulationDomain.xSize()) * 0.5_r - spacing;
   ...
   auto boundary = createBoundary(ps,
                                  ...
                                  real_t(simulationDomain.xSize()) * 0.5_r,
                                  Vec3(0,0,1));
   WALBERLA_UNUSED(boundary);
   //boundary->setAngularVelocity(Vec3(0_r, 0_r, rotationSpeed));
   ...
   auto box0                       = ps->create();
   box0->getPositionRef()          = Vec3(simulationDomain.xSize() * 0.5_r,0.0,0.0);
...
//   box0->getRotationRef().rotate(Vec3(0,0,1), -math::pi * 0.15 );
   ...
   auto box1                       = ps->create();
   box1->getPositionRef()          = Vec3(simulationDomain.xSize() * 0.5_r,0.0,0.0);
...
//   box1->getRotationRef().rotate(Vec3(0,0,1), -math::pi * 0.15 );
   ...
   dRot = Rot3(Vec3(0_r, 0_r, math::pi * 0.5));
   box1->setPosition( origin + dRot.getMatrix() * dp );
   box1->getRotationRef().rotate(dRot);

   auto box2                       = ps->create();
   box2->getPositionRef()          = Vec3(simulationDomain.xSize() * 0.5_r,0.0,0.0);
...
//   box2->getRotationRef().rotate(Vec3(0,0,1), -math::pi * 0.15 );
   ...
   dRot = Rot3(Vec3(0_r, 0_r, math::pi));
   box2->setPosition( origin + dRot.getMatrix() * dp );
   box2->getRotationRef().rotate(dRot);

   auto box3                       = ps->create();
   box3->getPositionRef()          = Vec3(simulationDomain.xSize() * 0.5_r,0.0,0.0);
...
//   box3->getRotationRef().rotate(Vec3(0,0,1), -math::pi * 0.15 );
   ...
   dRot = Rot3(Vec3(0_r, 0_r, math::pi * 1.5_r));
...
//   vtkOutput->addOutput("scale", select_scaling);
   ...
   dRot = Rot3(Vec3(0_r, 0_r, rotationSpeed) * dt);
         ...
         } else
         {
            if (collision_detection::isInInteractionDistance(idx1, idx2, ac))
            {
               if (double_cast(idx1, idx2, ac, acd, ac ))
               {
                  ...
                  {
                     auto meff = 1_r / (ac.getInvMass(idx1) + ac.getInvMass(idx2));
