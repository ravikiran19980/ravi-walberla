...
inline ThermalExpansion::ThermalExpansion(const uint_t numParticleTypes)
{
   ...
   linearExpansionCoefficient_.resize(numParticleTypes, 0_r);
}
                                         ...
                                         Accessor& ac) const
{
   ...
   const auto Tc = ac.getTemperature(p_idx)-273_r;
   ac.setRadiusAtTemperature(p_idx, ac.getRadius273K(p_idx) * (1_r + Tc * getLinearExpansionCoefficient(ac.getType(p_idx))));
