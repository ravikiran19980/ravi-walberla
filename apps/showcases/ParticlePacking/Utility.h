...
{
   return std::cbrt(3_r / ( 4_r * math::pi) * volume);
}

real_t diameterFromSphereVolume(real_t volume)
{
   return 2_r * radiusFromSphereVolume(volume);
}
...
std::vector<real_t> transferMassFractionsToParticleNumbersFromAvgVolumes(const std::vector<real_t>& massFractions, const std::vector<real_t>& avgVolumePerSizeFraction,
                                                                         real_t totalMass = 1_r, real_t density = 1_r )
{
   ...
   std::vector<real_t> particleNumbers(massFractions.size(), 0_r);
...
std::vector<real_t> transferMassFractionsToParticleNumbers(const std::vector<real_t>& massFractions, const std::vector<real_t>& diameters,
                                                           real_t normalVolume = math::pi / 6_r, real_t totalMass = 1_r, real_t density = 1_r )
{
   ...
   std::vector<real_t> avgVolumePerSizeFraction(massFractions.size(), 0_r);
   for(uint_t n = 0; n < massFractions.size(); ++n )
   {
                  ...
                  const Vec3& normal)
{
   ...
   p->getBaseShapeRef()->updateMassAndInertia(1_r);
