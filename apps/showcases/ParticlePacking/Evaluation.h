...
public:
   ...
   shapeBins_(shapeBins), shapeEvaluators_(shapeEvaluators) {
      bool areBinsAscending = (sizeBins[1] - sizeBins[0]) > 0_r;
      if(!areBinsAscending) std::reverse(sizeBins_.begin(), sizeBins_.end());

      massFractionHistogram_ = std::vector<real_t>(sizeBins.size() + 1,0_r);
   ...
   void evaluate()
   {
      ...
      {
         auto total = std::accumulate(massFractionHistogram_.begin(), massFractionHistogram_.end(), 0_r);
   ...
   void clear()
   {
      for(auto& h: massFractionHistogram_) h = 0_r;
...
public:
         ...
         : layerHeight_(layerHeight)
   {
      if(domainSetup == "container") {
         layerVolume_ = simulationDomain.xSize() * simulationDomain.xSize() * math::pi / 4_r * layerHeight;
      }
      else {
         layerVolume_ = simulationDomain.xSize() * simulationDomain.ySize() * layerHeight;
      }
      porosityPerLayer_ = std::vector<real_t>(uint_c(simulationDomain.zSize() / layerHeight), 0_r);
      radiusPerLayer_ = std::vector<real_t>(uint_c(simulationDomain.zSize() / layerHeight), 0_r);
   }
   ...
   void printToFile(std::string fileName)
   {
      WALBERLA_ROOT_SECTION()
      {
         ...
         for(uint_t i = 0; i < porosityPerLayer_.size(); ++i)
         {
            file << layerHeight_ * (0.5_r+real_c(i)) << " " << porosityPerLayer_[i] << " " << radiusPerLayer_[i] << "\n";
         }
         file.close();
      }
   }
...
private:
   ...
   real_t calculateSphericalSegmentVolume(real_t lowerLayerHeight, real_t upperLayerHeight, real_t radius) const
   {
      ...
      real_t a1 = std::sqrt(std::max(radius*radius - r1*r1, 0_r));
      real_t a2 = std::sqrt(std::max(radius*radius - r2*r2, 0_r));
      real_t h = r2-r1;
      // wikipedia Kugelschicht
      return math::pi * h / 6_r * (3_r*a1*a1 + 3_r*a2*a2 + h*h);
   }
...
public:
   ...
   void printToFile(std::string fileName)
   {
      WALBERLA_ROOT_SECTION()
      {
         ...
         for(uint_t i = 0; i < contactsPerLayer_.size(); ++i)
         {
            file << layerHeight_ * (0.5_r+real_c(i)) << " " << contactsPerLayer_[i]
