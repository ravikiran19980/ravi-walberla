...
class NeumannDomainBoundary
{
 public:
   ...
   {
      order_[stencil::D3Q6::idx[direction]] = order;
   }
   ...
   {
      value_[stencil::D3Q6::idx[direction]] = value;
   }
   ...
   for (auto block = blocks_.begin(); block != blocks_.end(); ++block)
   {
      ...
      if (includeBoundary_[stencil::D3Q6::idx[stencil::W]] && blocks_.atDomainXMinBorder(*block))
      {
         apply(p,
               ...
               cell_idx_t(1), cell_idx_t(0), cell_idx_t(0), order_[stencil::D3Q6::idx[stencil::W]],
               value_[stencil::D3Q6::idx[stencil::W]], dx_[stencil::D3Q6::idx[stencil::W]]);
      }
      if (includeBoundary_[stencil::D3Q6::idx[stencil::E]] && blocks_.atDomainXMaxBorder(*block))
      {
         apply(p,
               CellInterval(cell_idx_c(p->xSize()), cell_idx_t(0), cell_idx_t(0), cell_idx_c(p->xSize()),
                            cell_idx_c(p->ySize()) - cell_idx_t(1), cell_idx_c(p->zSize()) - cell_idx_t(1)),
               cell_idx_t(-1), cell_idx_t(0), cell_idx_t(0), order_[stencil::D3Q6::idx[stencil::E]],
               value_[stencil::D3Q6::idx[stencil::E]], dx_[stencil::D3Q6::idx[stencil::E]]);
      }
      ...
      if (includeBoundary_[stencil::D3Q6::idx[stencil::S]] && blocks_.atDomainYMinBorder(*block))
      {
         apply(p,
               ...
               cell_idx_t(0), cell_idx_t(1), cell_idx_t(0), order_[stencil::D3Q6::idx[stencil::S]],
               value_[stencil::D3Q6::idx[stencil::S]], dx_[stencil::D3Q6::idx[stencil::S]]);
      }
      if (includeBoundary_[stencil::D3Q6::idx[stencil::N]] && blocks_.atDomainYMaxBorder(*block))
      {
         apply(p,
               ...
               cell_idx_t(0), cell_idx_t(-1), cell_idx_t(0), order_[stencil::D3Q6::idx[stencil::N]],
               value_[stencil::D3Q6::idx[stencil::N]], dx_[stencil::D3Q6::idx[stencil::N]]);
      }
      ...
      if (includeBoundary_[stencil::D3Q6::idx[stencil::B]] && blocks_.atDomainZMinBorder(*block))
      {
         apply(p,
               ...
               cell_idx_t(0), cell_idx_t(0), cell_idx_t(1), order_[stencil::D3Q6::idx[stencil::B]],
               value_[stencil::D3Q6::idx[stencil::B]], dx_[stencil::D3Q6::idx[stencil::B]]);
      }
      if (includeBoundary_[stencil::D3Q6::idx[stencil::T]] && blocks_.atDomainZMaxBorder(*block))
      {
         apply(p,
               CellInterval(cell_idx_t(0), cell_idx_t(0), cell_idx_c(p->zSize()),
                            ...
                            cell_idx_c(p->zSize())),
               cell_idx_t(0), cell_idx_t(0), cell_idx_t(-1), order_[stencil::D3Q6::idx[stencil::T]],
               value_[stencil::D3Q6::idx[stencil::T]], dx_[stencil::D3Q6::idx[stencil::T]]);
      }
   }
}

template< typename PdeField >
void NeumannDomainBoundary< PdeField >::apply(PdeField* p, const CellInterval& interval, const cell_idx_t cx,
                                              const cell_idx_t cy, const cell_idx_t cz, const uint_t order,
                                              const real_t value, const real_t dx) const
{
   if (order == uint_t(1))
   {
      ...
      if (isIdentical(value, real_t(0)))
      {
         WALBERLA_FOR_ALL_CELLS_IN_INTERVAL_XYZ(
            ...
            const real_t boundaryValue =
               pBoundary + real_c(0.5) * (pBoundary - pInner); // extrapolation of value _on_ the boundary
...
class Neumann : public Boundary< flag_t >
{
 ...
 public:
   ...
   {
      return make_shared< NeumannBC >(config);
   }
    ...
    * \param blocks
    *******************************************************************************************************************/
   inline Neumann(const BoundaryUID& boundaryUID, const FlagUID& uid, Field_T* const rhsField,
                  const StencilField_T* const stencilField, StencilField_T* const adaptBCStencilField,
                  FlagField< flag_t >* const flagField, const StructuredBlockStorage& blocks);
   ...
   inline void registerCell(const flag_t, const cell_idx_t x, const cell_idx_t y, const cell_idx_t z,
   ...
   inline void registerCells(const flag_t, const CellIterator& begin, const CellIterator& end,
                             const BoundaryConfiguration& neumannBC);
   ...
   inline const real_t& getValue(const cell_idx_t x, cell_idx_t y, cell_idx_t z) const
   {
      return neumannBC_->get(x, y, z);
   }
...
template< typename Stencil_T, typename flag_t >
inline Neumann< Stencil_T, flag_t >::Neumann(const BoundaryUID& boundaryUID, const FlagUID& uid,
                                             Field_T* const rhsField, const StencilField_T* const stencilField,
                                             StencilField_T* const adaptBCStencilField,
                                             FlagField< flag_t >* const flagField, const StructuredBlockStorage& blocks)
   : Boundary< flag_t >(boundaryUID), uid_(uid), formerNeumann_(flagField->getOrRegisterFlag("FormerNeumann")),
     numDirtyCells_(std::numeric_limits< uint_t >::max()), rhsField_(rhsField), stencilField_(stencilField),
     adaptBCStencilField_(adaptBCStencilField), flagField_(flagField)
{
   ...
   WALBERLA_FOR_ALL_CELLS_XYZ(
      stencilField_, for (auto dir = Stencil_T::begin(); dir != Stencil_T::end(); ++dir) {
         WALBERLA_ASSERT_IDENTICAL(stencilField_->get(x, y, z, dir.toIdx()),
                                   adaptBCStencilField_->get(x, y, z, dir.toIdx()));
      })
#endif

   neumannBC_ =
      make_shared< Field_T >(rhsField_->xSize(), rhsField_->ySize(), rhsField_->zSize(), uint_t(1), field::zyxf);

   for (auto d = Stencil_T::beginNoCenter(); d != Stencil_T::end(); ++d)
   {
      dx_[d.toIdx()] =
         Vector3< real_t >(real_c(stencil::cx[d.toIdx()]) * blocks.dx(), real_c(stencil::cy[d.toIdx()]) * blocks.dy(),
                           real_c(stencil::cz[d.toIdx()]) * blocks.dz())
            .sqrLength();
...
template< typename Stencil_T, typename flag_t >
template< typename Buffer_T >
inline void Neumann< Stencil_T, flag_t >::packCell(Buffer_T& buffer, const cell_idx_t x, const cell_idx_t y,
...
template< typename Stencil_T, typename flag_t >
template< typename Buffer_T >
inline void Neumann< Stencil_T, flag_t >::registerCell(Buffer_T& buffer, const flag_t, const cell_idx_t x,
...
template< typename Stencil_T, typename flag_t >
inline void Neumann< Stencil_T, flag_t >::registerCell(const flag_t, const cell_idx_t x, const cell_idx_t y,
                                                       const cell_idx_t z, const BoundaryConfiguration& neumannBC)
{
...
template< typename Stencil_T, typename flag_t >
inline void Neumann< Stencil_T, flag_t >::registerCells(const flag_t, const CellInterval& cells,
                                                        const BoundaryConfiguration& neumannBC)
{
   WALBERLA_ASSERT_EQUAL(dynamic_cast< const NeumannBC* >(&neumannBC), &neumannBC);
   WALBERLA_ASSERT_NOT_NULLPTR(neumannBC_);
...
template< typename Stencil_T, typename flag_t >
template< typename CellIterator >
inline void Neumann< Stencil_T, flag_t >::registerCells(const flag_t, const CellIterator& begin,
                                                        const CellIterator& end, const BoundaryConfiguration& neumannBC)
{
   WALBERLA_ASSERT_EQUAL(dynamic_cast< const NeumannBC* >(&neumannBC), &neumannBC);
   WALBERLA_ASSERT_NOT_NULLPTR(neumannBC_);
...
template< typename Stencil_T, typename flag_t >
void Neumann< Stencil_T, flag_t >::unregisterCell(const flag_t, const cell_idx_t nx, const cell_idx_t ny,
                                                  const cell_idx_t nz)
{
   ...
   for (auto d = Stencil_T::begin(); d != Stencil_T::end(); ++d)
   {
      ...
      if (adaptBCStencilField_->isInInnerPart(domainCell))
      {
         ...
         adaptBCStencilField_->get(domainCell, Stencil_T::idx[stencil::C]) -=
            adaptBCStencilField_->get(domainCell, d.toIdx());
      }
   }
}
...
template< typename Stencil_T, typename flag_t >
#ifndef NDEBUG
inline void Neumann< Stencil_T, flag_t >::treatDirection(const cell_idx_t x, const cell_idx_t y, const cell_idx_t z,
                                                         const stencil::Direction dir, const cell_idx_t nx,
                                                         const cell_idx_t ny, const cell_idx_t nz, const flag_t mask)
#else
inline void Neumann< Stencil_T, flag_t >::treatDirection(const cell_idx_t x, const cell_idx_t y, const cell_idx_t z,
                                                         const stencil::Direction dir, const cell_idx_t nx,
                                                         const cell_idx_t ny, const cell_idx_t nz,
                                                         const flag_t /*mask*/)
#endif
{
   ...
   WALBERLA_ASSERT_EQUAL(mask & this->mask_,
                         this->mask_); // only true if "this->mask_" only contains one single flag, which is the case
                                       // for the current implementation of this boundary condition (Neumann)

   // WALBERLA_LOG_DEVEL("Neumann treatment at: " << Cell(x,y,z));

   // Adapt RHS to Neumann BC //
   rhsField_->get(x, y, z) -= stencilField_->get(x, y, z, Stencil_T::idx[dir]) * dx_[Stencil_T::idx[dir]] *
                              neumannBC_->get(nx, ny, nz); // possibly utilize that off-diagonal entries -1 anyway
   ...
   if (numDirtyCells_ > 0)
   {
      ...
      adaptBCStencilField_->get(x, y, z, Stencil_T::idx[stencil::C]) +=
         adaptBCStencilField_->get(x, y, z, Stencil_T::idx[dir]);
