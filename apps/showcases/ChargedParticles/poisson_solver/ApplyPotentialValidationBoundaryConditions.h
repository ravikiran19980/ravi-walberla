...
void applyPotentialBoundaryValue(IBlock* block, PdeField* p, const CellInterval& interval, const cell_idx_t cx,
                                 const cell_idx_t cy, const cell_idx_t cz, BoundaryCondition& e,
                                 StructuredBlockStorage& blocks, const math::AABB& domainAABB, real_t charge,
                                 real_t epsilon, real_t dx_[])
{
   real_t funcVal;
   WALBERLA_FOR_ALL_CELLS_IN_INTERVAL_XYZ(
      ...
      if (e.isDirichletType()) {
         ...
         {
            const real_t vdx = (funcVal * dx_[stencil::D3Q6::idx[e.getDirection()]]); // value * dx
