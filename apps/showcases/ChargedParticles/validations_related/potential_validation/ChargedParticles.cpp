...
//    using LatticeModel_T = lbm::D3Q19< lbm::collision_model::TRT >;
    ...
    //using CollisionModelType = typename std::conditional<useSRT, lbm::collision_model::SRT, lbm::collision_model::TRT>::type;
    //using LatticeModel_T = lbm::D3Q19<CollisionModelType>;
...
// Helper function to create the Type object
//    template<typename Type, typename... Args>
//    Type *createTypeObject(Args &&... args) {
...
//   }
    ...
    public:
        ...
        using Type = BoundaryHandling<FlagField_T, Stencil_T, FreeSlip_T, Inflow_T, Outflow_T>;
        //using Type = BoundaryHandling<FlagField_T, Stencil_T, NoSlip_T, Inflow_T, Outflow_T>;

        //using Type = typename std::conditional<useNoSlip,
        //        BoundaryHandling<FlagField_T, Stencil_T, NoSlip_T, Inflow_T, Outflow_T>,
        //        BoundaryHandling<FlagField_T, Stencil_T, FreeSlip_T, Inflow_T, Outflow_T>>::type;
        ...
        Type *operator()(IBlock *const block, const StructuredBlockStorage *const storage) const {
            ...
            /*Type *handling;
            //if constexpr (useNoSlip) {  // executes if true
                handling = createTypeObject<Type>("moving obstacle boundary handling", flagField, fluid,
           ...
           // else {  //executes if false
            ...
            const auto sliptype  = flagField->getFlag(FreeSlip_Flag);  // in this case no slip
            //const auto sliptype = flagField->getFlag(NoSlip_Flag) ;
            ...
            handling->forceBoundary(sliptype, bottom);
            ...
            handling->forceBoundary(sliptype, top);
            ...
            if (!periodicInX_) {
                ...
                handling->forceBoundary(sliptype, left);
                ...
                handling->forceBoundary(sliptype, right);
            }
            ...
            if (!periodicInY_) {
                ...
                handling->forceBoundary(sliptype, front);
                ...
                handling->forceBoundary(sliptype, back);
            }
...
//*******************************************************************************************************************
    int main(int argc, char** argv)
    {
        ...
        Vector3<real_t> Spacing(particleGenerationSpacing_x,particleGenerationSpacing_y,particleGenerationSpacing_z);
        ...
        WALBERLA_LOG_INFO_ON_ROOT(" - particle generation spacing = " << particleGenerationSpacing_x << " " << particleGenerationSpacing_y << " "<< particleGenerationSpacing_z);
        ...
        //auto poissonSolver = PoissonSolver< DAMPED_JACOBI,true >(/* src */ potentialFieldID, /* dst */ potentialFieldCopyID,
        //                                                       /* rhs */ chargeDensityFieldID, blocks, uint_c(1000),
        //                                                    real_c(1e-3), uint_c(1000));

        auto poissonSolver = PoissonSolver< DAMPED_JACOBI >(/* src */ potentialFieldID, /* dst */ potentialFieldCopyID,
                /* rhs */ chargeDensityFieldID,/*domainAABB*/ simulationDomain, blocks, vacuum_permitivity ,maxCharge, uint_c(50000),
        ...
        Vector3<real_t> pointOfReference {generationDomain.center()[0],generationDomain.center()[1],generationDomain.center()[2]};
        WALBERLA_LOG_INFO_ON_ROOT("pointofreference is:" << pointOfReference);

        for (auto pt : grid_generator::SCGrid(generationDomain, generationDomain.center() + Vector3(0, 0, 200), Spacing))
        {
            if (rpdDomain->isContainedInProcessSubdomain(uint_c(mpi::MPIManager::instance()->rank()), pt))
            {
                ...
                //WALBERLA_LOG_INFO_ON_ROOT("charge is:" << maxCharge);
                //std::cout << "charge is:" << maxCharge;
            }
        }
        ...
        // ADD DATA TO BLOCKS //
        ////////////////////////
        //LatticeModel_T latticeModel = LatticeModel_T(omega);  ->>> previous one for srt

        //real_t lambda_e = lbm::collision_model::TRT::lambda_e( omega );
        //real_t lambda_d = (true) ? lambda_e : lbm::collision_model::TRT::lambda_d( omega, magicNumber );


        LatticeModel_T latticeModel = LatticeModel_T(omega);
        //LatticeModel_T  latticeModel(lbm::collision_model::TRT(lambda_e,lambda_d));
        ...
        // true -> for noSlip boundary and false -> for FreeSlip boundary
        //const bool useNoSlip = false;
        ...
        Vector3< real_t > externalForce(real_t(0),real_t(0),-1.74);   // dominik thesis
        lbm_mesapd_coupling::AddForceOnParticlesKernel addexternalForce(externalForce);
        ...
        ResetElectrostaticForceKernel resetElectrostaticForce;
        ...
        if (vtkSpacingParticles != uint_t(0))
        {
            ...
            particleVtkOutput->addOutput< mesa_pd::data::SelectParticleElectrostaticForce >("ElectrostaticForce");
            particleVtkOutput->addOutput< mesa_pd::data::SelectParticleHydrodynamicForce >("HydrodynamicForce");
            particleVtkOutput->addOutput< mesa_pd::data:: SelectParticleForce>("TotalForce");
        ...
        if (vtkSpacingFluid != uint_t(0))
        {
            ...
            electrostaticForceFieldVTK->addCellDataWriter(
                    make_shared< field::VTKWriter< VectorField_T > >(electrostaticForceFieldID, "Electrostatic force field"));
            ...
            electrostaticPotentialVTK->addCellDataWriter(
                    make_shared< field::VTKWriter< ScalarField_T > >(potentialFieldID, "Electrostatic potential"));
        ...
        for (uint_t timeStep = 0; timeStep < numTimeSteps; ++timeStep)
        {
            ...
            for (auto subCycle = uint_t(0); subCycle < numberOfParticleSubCycles; ++subCycle)
            {
                ...
                // TODO:     write and add addElectrostaticInteraction, see above  ---> completed
                ...
                //ps->forEachParticle(useOpenMP, mesa_pd::kernel::SelectLocal(), *accessor, addexternalForce, *accessor);
        ...
        real_t errorMax = computeErrorMax(blocks, potentialFieldID,simulationDomain,diameter/2 ,vacuum_permitivity,maxCharge);
        WALBERLA_LOG_INFO_ON_ROOT("Infinity Norm error is: " << errorMax);

        real_t errorL2 = computeErrorL2(blocks, potentialFieldID,simulationDomain,diameter/2 ,vacuum_permitivity,maxCharge);
        WALBERLA_LOG_INFO_ON_ROOT("L2 Norm error is: " << errorL2);

        //real_t ElectrostaticForceAnalytical = (10* maxCharge)/(simulationDomain.max()[2] - simulationDomain.min()[2]);
        //std::cout << "Analytical value of Electrostatic Force:" << " " << ElectrostaticForceAnalytical << std::endl;
