    ...
    public:
        ...
        Type *operator()(IBlock *const block, const StructuredBlockStorage *const storage) const {
           ...
           // else {  //executes if false
            ...
            const auto sliptype  = flagField->getFlag(NoSlip_Flag);  // in this case no slip
            //const auto sliptype = (useNoSlip) ? flagField->getFlag(NoSlip_Flag) : flagField->getFlag(FreeSlip_Flag);
...
//*******************************************************************************************************************
    int main(int argc, char** argv)
    {
        ...
        const Vector3< real_t > inflowVec(0_r, 0_r, 0_r);
        ...
        Vector3< uint_t > numberOfBlocksPerDirection(numXBlocks, numYBlocks, numZBlocks);
        ...
        auto poissonSolver = PoissonSolver< DAMPED_JACOBI >(/* src */ potentialFieldID, /* dst */ potentialFieldCopyID,
                /* rhs */ chargeDensityFieldID,/*domainAABB*/ simulationDomain, blocks, vacuum_permitivity ,maxCharge, uint_c(1000),
                                                                      real_c(1e-12), uint_c(1000), boundaryconditions);
        ...
        if (vtkSpacingFluid != uint_t(0))
        {
            ...
            //pdfFieldVTK->addBeforeFunction(commFunction);
        ...
        for (uint_t timeStep = 0; timeStep < timesteps; ++timeStep)
        {
            ...
            for (auto subCycle = uint_t(0); subCycle < numberOfParticleSubCycles; ++subCycle)
            {
                ...
                ps->forEachParticlePairHalf(
                        useOpenMP, mesa_pd::kernel::ExcludeInfiniteInfinite(), *accessor,
                        [&collisionResponse, &rpdDomain, numberOfParticleSubCycles, coefficientOfRestitution, particleCollisionTime,
                            ...
                            if (double_cast(idx1, idx2, ac, acd, ac))
                            {
                                if (contact_filter(acd.getIdx1(), acd.getIdx2(), ac, acd.getContactPoint(), *rpdDomain))
                                {
                                    ...
                                    collisionResponse(acd.getIdx1(), acd.getIdx2(), ac, acd.getContactPoint(), acd.getContactNormal(),
                                                      acd.getPenetrationDepth(), numberOfParticleSubCycles);
                                }
                            }
                        },
                        *accessor);
            ...
            //WALBERLA_LOG_INFO_ON_ROOT("sphere posiiton is:" <<  logger.getPosition() << "velocity is:"<<" " << logger.getMaxVelocity() << " " << "numparticles:" << particleInfo.numParticles);
            WALBERLA_LOG_INFO_ON_ROOT("sphere posiiton logger:" << " "<<  logger.getPosition() << " "<<  "sphere position info:"<<" " << particleInfo.heightOfMass<< "velocity logger:"<<" " << logger.getMaxVelocity() << " "<< "velocity info is:"<<" " << particleInfo.averageVelocity);
        ...
        real_t relErr_LBM = std::fabs(expectedSettlingVelocity - logger.getMaxVelocity()) / expectedSettlingVelocity;
        real_t relErr_SI = std::fabs(expectedSettlingVelocity_SI - (logger.getMaxVelocity()*dx_SI/dt_SI)) / expectedSettlingVelocity_SI;

        WALBERLA_LOG_INFO_ON_ROOT("Expected maximum settling velocity in LBM units: " << expectedSettlingVelocity);
        WALBERLA_LOG_INFO_ON_ROOT("Simulated maximum settling velocity in LBM units: " << logger.getMaxVelocity());

        WALBERLA_LOG_INFO_ON_ROOT("Expected maximum settling velocity in SI units: " << expectedSettlingVelocity_SI);
        WALBERLA_LOG_INFO_ON_ROOT("Simulated maximum settling velocity in SI units: " << (logger.getMaxVelocity()*dx_SI/dt_SI));

        WALBERLA_LOG_INFO_ON_ROOT("Relative error for LBM velocities: " << relErr_LBM);
        WALBERLA_LOG_INFO_ON_ROOT("Relative error for SI velocities: " << relErr_SI);
