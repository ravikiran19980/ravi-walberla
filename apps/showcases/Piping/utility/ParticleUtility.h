...
{
   if ((spherePosition[0] + sphereRadius < boxPosition[0] - boxEdgeLength[0] / 2_r) ||
       (spherePosition[1] + sphereRadius < boxPosition[1] - boxEdgeLength[1] / 2_r) ||
       (spherePosition[2] + sphereRadius < boxPosition[2] - boxEdgeLength[2] / 2_r) ||
       (spherePosition[0] - sphereRadius > boxPosition[0] + boxEdgeLength[0] / 2_r) ||
       (spherePosition[1] - sphereRadius > boxPosition[1] + boxEdgeLength[1] / 2_r) ||
       (spherePosition[2] - sphereRadius > boxPosition[2] + boxEdgeLength[2] / 2_r))
   {
      return false;
                         ...
                         const mesa_pd::Vec3& boxPosition, const mesa_pd::Vec3& boxEdgeLength, real_t& maxDiameter)
{
   ...
   real_t maxParticleDiameter = 0_r;
   ...
   while (std::getline(fileIss, line))
   {
      ...
      minParticleDiameter = std::min(2_r * radius, minParticleDiameter);
      maxParticleDiameter = std::max(2_r * radius, maxParticleDiameter);
...
auto createPlane(mesa_pd::data::ParticleStorage& ps, const mesa_pd::Vec3& pos, const mesa_pd::Vec3& normal)
{
   ...
   p0->getBaseShapeRef()->updateMassAndInertia(1_r);
               ...
               const bool movingBucket)
{
   ...
   if (movingBucket)
   {
      ...
      p0->getBaseShapeRef()->updateMassAndInertia(2.0_r);
   }
   else
   {
                     ...
                     const real_t& particleDensityRatio, const real_t& gravitationalAcceleration, const bool& useOpenMP)
{
   ...
   const real_t timeStepSizeParticles = 10_r;
   ...
   ps->forEachParticle(
      ...
      [](const size_t idx, auto& ac) {
         ac.setLinearVelocity(idx, Vector3(0_r));
         ac.setAngularVelocity(idx, Vector3(0_r));
      },
      *accessor);
   syncNextNeighborFunc(*ps, domain);
}
