...
{
   std::fill(errorHistory_.data(), errorHistory_.data() + sizeof(errorHistory_) / sizeof(real_t), 0_r);
}
...
{
   static const real_t ONE_OVER_SIX = 1_r / 6_r;
   const real_t error               = commandVariable_ - controlledVariable;

   const real_t d =
      (error + 3_r * errorHistory_[0] - 3_r * errorHistory_[1] - errorHistory_[2]) * ONE_OVER_SIX;
