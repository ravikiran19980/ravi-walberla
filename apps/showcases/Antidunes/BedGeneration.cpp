...
int main(int argc, char** argv)
{
   ...
   // BlockForest
   math::AABB simulationDomain_SI(0.0_r, 0.0_r, 0.0_r, domainSize_SI[0], domainSize_SI[1],
                                  domainSize_SI[2]);
   ...
   // Init spheres
   real_t minDiameter_SI = diameter_SI * 0.9_r;
   real_t maxDiameter_SI = diameter_SI * 1.1_r;
        ...
        grid_generator::SCGrid(generationDomain_SI, Vec3(generationSpacing_SI) * real_c(0.5), generationSpacing_SI))
   {
      ...
      if (!domain->isContainedInLocalSubdomain(pt, 0_r)) continue;
      auto p                       = ps->create();
      p->getPositionRef()          = pt;
      p->getInteractionRadiusRef() = diameter * 0.5_r;
      ...
      p->setLinearVelocity(Vec3(0.1_r * math::realRandom(-initialVelocity_SI, initialVelocity_SI),
                                0.1_r * math::realRandom(-initialVelocity_SI, initialVelocity_SI),
                                -initialVelocity_SI));
   ...
   {
      auto diameter = math::realRandom< real_t >(minDiameter_SI, maxDiameter_SI);
      auto zCoord   = math::realRandom< real_t >(1e-10_r, diameter_SI * bottomLayerOffsetFactor);
      Vec3 position{ pt[0], pt[1] * bottomLayerYStretchFactor, zCoord };

      if (!domain->isContainedInLocalSubdomain(position, 0_r)) continue;
      auto p                       = ps->create();
   ...
   createPlane(*ps, simulationDomain_SI.minCorner(), Vec3(0_r, 0_r, 1_r));
   createPlane(*ps, simulationDomain_SI.maxCorner(), Vec3(0_r, 0_r, -1_r));
   ...
   real_t kappa = 2_r * (1_r - poissonsRatio) / (2_r - poissonsRatio); // from Thornton et al
   ...
   real_t maxVelocity         = 0_r;
   real_t maxHeight           = 0_r;
