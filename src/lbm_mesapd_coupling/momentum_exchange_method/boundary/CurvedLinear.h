...
#endif
{
   ...
   if( flagField_->isPartOfMaskSet( xff, yff, zff, domainMask_ ) )
   {
      ...
      const real_t tolerance = 1e-4_r * lengthScalingFactor_;
      ...
      const real_t kappa0 = ( 1_r - 2_r * delta ) / ( 1_r + 2_r * delta );
   ...
   if( LatticeModel_T::compressible )
   {
      ...
      pdf_new -= 3.0_r * alpha * density * LatticeModel_T::w[ Stencil_T::idx[dir] ] *
   ...
   {
      pdf_new -= 3.0_r * alpha * LatticeModel_T::w[ Stencil_T::idx[dir] ] *
                 ( real_c( stencil::cx[ dir ] ) * boundaryVelocity[0] +
                   real_c( stencil::cy[ dir ] ) * boundaryVelocity[1] +
                   real_c( stencil::cz[ dir ] ) * boundaryVelocity[2] );
   }
