...
{
   WALBERLA_ASSERT_FLOAT_EQUAL(interactionNormal12.length(), 1_r, "InteractionNormal has to be normalized!");
   ...
   lubricationForce1 = Vector3<real_t>(0_r);
   lubricationTorque1 = Vector3<real_t>(0_r);

   if( gapSize < cutOffDistanceNormal )
   {
      // add lubrication force due to normal translation
      real_t epsres = cutOffDistanceNormal / radius1;
      Vector3<real_t> Fn1 = 6_r * walberla::math::pi * radius1 * dynamicFluidViscosity *
                            ( kappa * kappa / ( ( 1_r + kappa ) * ( 1_r + kappa ) ) ) * ( 1_r / epsilon - 1_r / epsres) *
   ...
   if( gapSize < cutOffDistanceTangentialTranslational )
   {
      ...
      Vector3<real_t> Ftt1 = 6_r * walberla::math::pi * radius1 * dynamicFluidViscosity *
                             ( - 4_r * kappa * (2_r + kappa + 2_r * kappa * kappa ) / ( 15_r * (1_r + kappa) * (1_r + kappa) * (1_r + kappa) ) ) * logEpsDivEpsres *
                             (u12 - math::dot(u12,e12) * e12);

      lubricationForce1 += Ftt1;

      Vector3<real_t> Ttt1 = 8_r * walberla::math::pi * radius1 * radius1 * dynamicFluidViscosity *
                             ( - kappa * (4_r + kappa) / (10_r * (1_r+kappa) * (1_r+kappa) ) ) * logEpsDivEpsres *
                             math::cross(e12, u12);
   ...
   if( gapSize < cutOffDistanceTangentialRotational )
   {
      ...
      real_t logEpsDivEpsres = std::log(epsilon/epsres);
      Vector3<real_t> Ftr1 = 6_r * walberla::math::pi * radius1 * radius1 * dynamicFluidViscosity *
                             (2_r * kappa * kappa / (15_r * (1_r + kappa) * (1_r + kappa) ) ) * logEpsDivEpsres *
                             math::cross(omega12 + 4_r / kappa * omega1 + 4_r * kappa * omega2, e12);

      lubricationForce1 += Ftr1;

      Vector3<real_t> tempOmega = omega1 + kappa * omega2 / 4_r;

      Vector3<real_t> Ttr1 = 8_r * walberla::math::pi * radius1 * radius1 * radius1 * dynamicFluidViscosity *
                             (2_r * kappa / ( 5_r * (1_r + kappa))) * logEpsDivEpsres *
                                                  ...
                                                  Vector3<real_t> & lubricationForce1, Vector3<real_t> & lubricationTorque1 )
{
   ...
   if( gapSize < cutOffDistanceTangentialTranslational )
   {
      ...
      real_t logEpsDivEpsres = std::log(epsilon/epsres);
      Vector3<real_t> Ftt1 = 6_r * walberla::math::pi * radius1 * dynamicFluidViscosity *
                             ( - 8_r / 15_r ) * logEpsDivEpsres *
                             (u12 - math::dot(u12,e12) * e12);

      lubricationForce1 += Ftt1;

      Vector3<real_t> Ttt1 = 8_r * walberla::math::pi * radius1 * radius1 * dynamicFluidViscosity *
                             ( - 1_r / 10_r ) * logEpsDivEpsres *
                             math::cross(e12, u12);
   ...
   if( gapSize < cutOffDistanceTangentialRotational )
   {
      ...
      real_t logEpsDivEpsres = std::log(epsilon/epsres);
      Vector3<real_t> Ftr1 = 6_r * walberla::math::pi * radius1 * radius1 * dynamicFluidViscosity *
                             ( 2_r / 15_r ) * logEpsDivEpsres *
                             math::cross(omega12, e12);

      lubricationForce1 += Ftr1;

      Vector3<real_t> Ttr1 = 8_r * walberla::math::pi * radius1 * radius1 * radius1 * dynamicFluidViscosity *
                             ( 2_r / 5_r ) * logEpsDivEpsres *
...
public:
   ...
   explicit LubricationCorrectionKernel( real_t dynamicFluidViscosity,
                                         ...
                                         real_t cutOffDistanceNormal = 2_r / 3_r,
                                         real_t cutOffDistanceTangentialTranslational = 0.5_r,
                                         real_t cutOffDistanceTangentialRotational = 0.5_r )
                           ...
                           ParticleAccessor_T& ac, const Vector3<real_t>& interactionNormal, const real_t& gapSize)
   {
      ...
      if( gapSize > 0_r)
      {
         ...
         Vector3<real_t> lubricationForce1(0_r);
         Vector3<real_t> lubricationTorque1(0_r);
         ...
         Vector3<real_t> lubricationForce2(0_r);
         Vector3<real_t> lubricationTorque2(0_r);
