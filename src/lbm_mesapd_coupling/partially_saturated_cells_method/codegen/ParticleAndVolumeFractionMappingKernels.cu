          ...
          startSamplingPoint.z + dx > minCornerSphere.z && startSamplingPoint.z < maxCornerSphere.z)
      {
         ...
         if (overlapFraction > 0.0)
         {
            ...
            calculateWeighting< Weighting_T >(&BsField.get(nOverlappingParticlesField.get()),
                                              BsField.get(nOverlappingParticlesField.get()), 1.0_r / omega);
                           ...
                           const double3 blockStart, const real_t dx, const id_t idxMapped)
{
   ...
   const double3 cellCenter = { (blockStart.x + (threadIdx.x + 0.5) * dx), (blockStart.y + (blockIdx.x + 0.5) * dx),
                                (blockStart.z + (blockIdx.y + 0.5) * dx) };
   const double3 cellMin    = { cellCenter.x - dx * 0.5_r, cellCenter.y - dx * 0.5_r,
                                cellCenter.z - dx * 0.5_r };
   const double3 cellMax    = { cellCenter.x + dx * 0.5_r, cellCenter.y + dx * 0.5_r,
                                cellCenter.z + dx * 0.5_r };

   const real_t xOverlap        = max(0_r, min(boxPositionMax.x, cellMax.x) - max(boxPositionMin.x, cellMin.x));
   const real_t yOverlap        = max(0_r, min(boxPositionMax.y, cellMax.y) - max(boxPositionMin.y, cellMin.y));
   const real_t zOverlap        = max(0_r, min(boxPositionMax.z, cellMax.z) - max(boxPositionMin.z, cellMin.z));
   const real_t overlapFraction = xOverlap * yOverlap * zOverlap / (dx * dx * dx);

   if (overlapFraction > 0_r)
   {
      assert(nOverlappingParticlesField.get() < MaxParticlesPerCell);
