                                      ...
                                      real_t* __restrict__ const positions, const double3 blockStart, const real_t dx)
{
   ...
   const real_t cellCenter[] = { (blockStart.x + (threadIdx.x + 0.5) * dx), // NOLINT(*-avoid-c-arrays)
                                 (blockStart.y + (blockIdx.x + 0.5) * dx), (blockStart.z + (blockIdx.y + 0.5) * dx) };
   ...
   {
      real_t particleVelocityAtWFPoint[] = { 0.0, 0.0, 0.0 }; // NOLINT(*-avoid-c-arrays)
