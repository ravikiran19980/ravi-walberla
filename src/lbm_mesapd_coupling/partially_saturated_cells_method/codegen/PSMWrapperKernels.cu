                                      ...
                                      real_t* __restrict__ const positions, const double3 blockStart, const real_t dx)
{
   ...
   {
      particleVelocitiesField.get(i * 3 + 0) = 0.0_r;
      particleVelocitiesField.get(i * 3 + 1) = 0.0_r;
      particleVelocitiesField.get(i * 3 + 2) = 0.0_r;
   }

   // Cell center is needed in order to compute the particle velocity at this WF point
   const real_t cellCenter[] = { real_t(blockStart.x + (threadIdx.x + 0.5_r) * dx), // NOLINT(*-avoid-c-arrays)
                                 real_t(blockStart.y + (blockIdx.x + 0.5_r) * dx),
                                 real_t(blockStart.z + (blockIdx.y + 0.5_r) * dx) };
   ...
   {
      real_t particleVelocityAtWFPoint[] = { 0.0_r, 0.0_r, 0.0_r }; // NOLINT(*-avoid-c-arrays)
