...
WALBERLA_LBM_CELLWISE_SWEEP_STREAM_COLLIDE_HEAD( WALBERLA_LBM_CELLWISE_SWEEP_SPECIALIZATION_MRT_1 )
{
   ...
   WALBERLA_FOR_ALL_CELLS_INCLUDING_GHOST_LAYER_XYZ( src, numberOfGhostLayersToInclude,
      ...
      if( this->filter(x,y,z) )
      {
         ...
         {
            const auto commonForceTerms = lm.forceModel().template directionIndependentTerms< LatticeModel_T >( x, y, z, velocity, rho + 1.0_r, collisionModel.omega(), collisionModel.omega_bulk(), collisionModel.omega_odd() );
            for( auto d = Stencil_T::begin(); d != Stencil_T::end(); ++d )
               dst->get( x, y, z, d.toIdx() ) += lm.forceModel().template forceTerm< LatticeModel_T >( x, y, z, velocity, rho + 1.0_r, commonForceTerms, LatticeModel_T::w[ d.toIdx() ], real_c(d.cx()), real_c(d.cy()), real_c(d.cz()), collisionModel.omega(), collisionModel.omega_bulk(), collisionModel.omega_odd() );
         }
      }
   ...
   WALBERLA_FOR_ALL_CELLS_INCLUDING_GHOST_LAYER_XYZ( src, numberOfGhostLayersToInclude,
      ...
      if( this->filter(x,y,z) )
      {
         ...
         if (!std::is_same_v< typename LatticeModel_T::ForceModel::tag, force_model::None_tag >)
         {
            const auto commonForceTerms = lm.forceModel().template directionIndependentTerms< LatticeModel_T >( x, y, z, velocity, rho + 1.0_r, collisionModel.omega(), collisionModel.omega_bulk(), collisionModel.omega_odd() );
            for( auto d = Stencil_T::begin(); d != Stencil_T::end(); ++d )
               src->get( x, y, z, d.toIdx() ) += lm.forceModel().template forceTerm< LatticeModel_T >( x, y, z, velocity, rho + 1.0_r, commonForceTerms, LatticeModel_T::w[ d.toIdx() ], real_c(d.cx()), real_c(d.cy()), real_c(d.cz()), collisionModel.omega(), collisionModel.omega_bulk(), collisionModel.omega_odd() );
         }
      }
