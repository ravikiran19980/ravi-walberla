...
UniformGPUScheme< Stencil >::UniformGPUScheme(const weak_ptr< StructuredBlockForest >& bf,
                                              const bool sendDirectlyFromGPU, const bool useLocalCommunication,
                                              const int tag)
   : blockForest_(bf), setupBeforeNextCommunication_(true), communicationInProgress_(false),
     sendFromGPU_(sendDirectlyFromGPU), useLocalCommunication_(useLocalCommunication),
     bufferSystemCPU_(mpi::MPIManager::instance()->comm(), tag),
     bufferSystemGPU_(mpi::MPIManager::instance()->comm(), tag), requiredBlockSelectors_(Set< SUID >::emptySet()),
...
UniformGPUScheme< Stencil >::UniformGPUScheme(const weak_ptr< StructuredBlockForest >& bf,
   ...
   : blockForest_(bf), setupBeforeNextCommunication_(true), communicationInProgress_(false),
     sendFromGPU_(sendDirectlyFromGPU), useLocalCommunication_(useLocalCommunication),
     bufferSystemCPU_(mpi::MPIManager::instance()->comm(), tag),
     bufferSystemGPU_(mpi::MPIManager::instance()->comm(), tag), requiredBlockSelectors_(requiredBlockSelectors),
...
void UniformGPUScheme< Stencil >::startCommunication()
{
   ...
   if (setupBeforeNextCommunication_ || currentBlockForestStamp != forestModificationStamp_) setupCommunication();
   ...
   // Start filling send buffers
   {
      for (auto& iBlock : *forest)
      {
         ...
         for (auto dir = Stencil::beginNoCenter(); dir != Stencil::end(); ++dir)
         {
            ...
            if (senderBlock->getNeighborhoodSectionSize(neighborIdx) == uint_t(0)) continue;

            if (!selectable::isSetSelected(senderBlock->getNeighborState(neighborIdx, uint_t(0)),
                                           requiredBlockSelectors_, incompatibleBlockSelectors_))
               continue;

            if (senderBlock->neighborExistsLocally(neighborIdx, uint_t(0)) && useLocalCommunication_)
            {
               auto receiverBlock =
                  dynamic_cast< Block* >(forest->getBlock(senderBlock->getNeighborId(neighborIdx, uint_t(0))));
            ...
            else
            {
               ...
               for (auto& pi : packInfos_)
               {
                  ...
                  if (!sendFromGPU_)
                  {
                     ...
                     WALBERLA_GPU_CHECK(
                        gpuMemcpyAsync(cpuDataPtr, gpuDataPtr, size, gpuMemcpyDeviceToHost, streams_[dirIdx]))
                  }
               }
            }
         }
         ...
         for (auto& header : headers_[recvInfo.rank()])
         {
            ...
            for (auto& pi : packInfos_)
            {
               ...
               WALBERLA_GPU_CHECK(gpuMemcpyAsync(gpuDataPtr, cpuDataPtr, size, gpuMemcpyHostToDevice,
                                                 streams_[Stencil::idx[header.dir]]))
               pi->unpack(stencil::inverseDir[header.dir], gpuDataPtr, block, streams_[Stencil::idx[header.dir]]);
            }
         }
      }
...
void UniformGPUScheme< Stencil >::setupCommunication()
{
   ...
   for (auto& iBlock : *forest)
   {
      ...
      for (auto dir = Stencil::beginNoCenter(); dir != Stencil::end(); ++dir)
      {
         ...
         WALBERLA_ASSERT(block->neighborhoodSectionHasEquallySizedBlock(neighborIdx), "Works for uniform setups only")
         ...
         if (block->neighborExistsLocally(neighborIdx, uint_t(0)) && useLocalCommunication_) continue;
