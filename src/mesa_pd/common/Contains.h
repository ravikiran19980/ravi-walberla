...
{
   return !((point - halfSpacePosition) * halfSpaceNormal > 0_r);
}
...
{
   return std::fabs(pointBF[0]) <= 0.5_r*edgeLengths[0] &&
          std::fabs(pointBF[1]) <= 0.5_r*edgeLengths[1] &&
          std::fabs(pointBF[2]) <= 0.5_r*edgeLengths[2];
}
...
bool isPointInsideConvexPolyhedronBF(const Vec3& point, const mesh::TriangleMesh& mesh)
{
   ...
   return std::none_of(mesh.faces().begin(),
                       ...
                       [&](auto fh)
                       {
                         ...
                         return (n[0] * (point[0] - pp[0]) + n[1] * (point[1] - pp[1]) + n[2] * (point[2] - pp[2]) >= 0_r);
                       });
}
#endif
