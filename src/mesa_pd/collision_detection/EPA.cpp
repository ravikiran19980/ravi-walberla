                 ...
                 real_t eps_rel )
{
   ...
   do {
      ...
      if(!current->isObsolete()) {
         ...
         WALBERLA_ASSERT_GREATER(farDist, 0.0_r, "EPA support mapping gave invalid point in expansion direction");
         ...
         if (bUseSphereOptimization_)
         {
            ...
            real_t radius2 = calculateCircle(support,
                  ...
                  epaVolume[(*current)[2]],
                  ctr);
            if(radius2 > 0.0_r){ //if a Sphere exists
               ...
               if (circle_dist*circle_dist <= upperBoundSqr &&
                   ...
                   (circle_dist*circle_dist) < 1.10_r * lowerBoundSqr &&
                   !floatIsEqual(center_len, 0.0_r) &&
                   circle_dist > 0.0_r) // In case of numerical errors, this can be the case
               {
                  const auto ilen = 1.0_r / center_len;
                  ctr *= -ilen;
                  ...
                  if(floatIsEqual((support % ctr).sqrLength()/support.sqrLength(), 0.0_r) &&
                     supp_dist*supp_dist <= upperBoundSqr &&
                     supp_dist*supp_dist >= lowerBoundSqr)
                  {
                     //Accept sphere
                     contactPoint = 0.5_r * (supportA.back() + supportB.back());
                     penetrationDepth = -supp_dist + 2.0_r * margin;
                     retNormal = -ctr;
         ...
         if(upperBoundSqr <= (1.0_r+eps_rel)*(1.0_r+eps_rel)*lowerBoundSqr
         ...
         if(epaVolume[(*firstTriangle)[0]] * firstTriangle->getNormal() < 0.0_r){
         ...
         for(; it != edgeBuffer.end(); ++it){
            ...
            if(epaVolume[(*newTriangle)[0]] * newTriangle->getNormal() < 0.0_r){
               //the whole triangle is on the wrong side of the origin.
               //This is an error.
               break;
            }
   ...
   contactPoint = 0.5_r * (wittnessA + wittnessB);

   //Penetration Depth
   penetrationDepth = -(current->getClosest().length() - 2.0_r * margin);
                                       ...
                                       real_t margin )
{
   ...
   case 2:
   {
      ...
      if( abs0 < abs1 && abs0 < abs2) {
         axis = Vec3(1.0_r, 0.0_r, 0.0_r);
      }
      else if( abs1 < abs0 && abs1 < abs2) {
         axis = Vec3(0.0_r, 1.0_r, 0.0_r);
      }
      else {
         axis = Vec3(0.0_r, 0.0_r, 1.0_r);
      }

      Vec3 direction1 = (d % axis).getNormalizedIfNotZero();
      Quat q(d, (2.0_r/3.0_r) * real_t(walberla::math::pi));
                                      ...
                                      const Vec3& p3 )
{
   ...
   if( (normal0T*p0 > 0.0_r) == (normal0T*p3 > 0.0_r) ) {
      return false;
   }
   Vec3 normal1T = (p2 -p1) % (p3-p1);
   if( (normal1T*p1 > 0.0_r) == (normal1T*p0 > 0.0_r) ) {
      return false;
   }
   Vec3 normal2T = (p3 -p2) % (p0-p2);
   if( (normal2T*p2 > 0.0_r) == (normal2T*p1 > 0.0_r) ) {
      return false;
   }
   Vec3 normal3T = (p0 -p3) % (p1-p3);
   return (normal3T*p3 > 0.0_r) != (normal3T*p2 > 0.0_r);
}
//*************************************************************************************************
                                   ...
                                   real_t margin )
{
   ...
   do{
      ...
      if( (normal0T*epaVolume[3] < dot_val) == (dot_val < 0.0_r) ) {
      ...
      if( (normal1T*epaVolume[0] < dot_val) == (dot_val < 0.0_r) ) {
      ...
      if( (normal2T*epaVolume[1] < dot_val) == (dot_val < 0.0_r) ) {
         pointIndexToRemove = 1;
      ...
      if( (normal3T*epaVolume[2] < dot_val) == (dot_val < 0.0_r) ) {
...
inline real_t EPA::calculateCircle(const Vec3& A, const Vec3& B, const Vec3& C,
   ...
   if(floatIsEqual(det, 0.0_r)){
      //no circle exists. Leave center untouched, and return -1.0
      return -1.0_r;
   }
   const real_t Alen = A.sqrLength();
   const real_t d1 = (Alen - B.sqrLength())*0.5_r;
   const real_t d2 = (Alen - C.sqrLength())*0.5_r;
   const real_t d3 = (Alen - D.sqrLength())*0.5_r;

   //Apply solution formula
   center = (1.0_r/det)*(d1 * (n2 % n3) + d2 * (n3 % n1) + d3 * (n1 % n2));
