...
inline NonLinearSpringDashpot::NonLinearSpringDashpot(const uint_t numParticleTypes, const real_t collisionTime)
{
   ...
   lnCORsqr_.resize(numParticleTypes * numParticleTypes, 0_r);
   meff_.resize(numParticleTypes * numParticleTypes, 0_r);
   stiffnessT_.resize(numParticleTypes * numParticleTypes, 0_r);
   dampingT_.resize(numParticleTypes * numParticleTypes, 0_r);
   frictionCoefficientStatic_.resize(numParticleTypes * numParticleTypes, 0_r);
   frictionCoefficientDynamic_.resize(numParticleTypes * numParticleTypes, 0_r);
                                            ...
                                            const real_t& dt) const
{
   ...
   if (p_idx1 != p_idx2)
   {
      ...
      WALBERLA_ASSERT_FLOAT_EQUAL(math::sqrLength(contactNormal), 1_r);
      ...
      Vec3 tangentialSpringDisplacement( 0_r );
      real_t impactVelocityMagnitude(0_r);
      ...
      const real_t A = 0.716_r;
      const real_t B = 0.830_r;
      const real_t C = 0.744_r;
      const real_t alpha = 1.111_r;
      const real_t tau_c0 = 3.218_r;
      const real_t nu = getLnCORsqr(ac.getType(p_idx1), ac.getType(p_idx2));
      const real_t lambda = (-0.5_r * C * nu + std::sqrt(0.25_r * C * C * nu * nu + alpha * alpha * tau_c0 * tau_c0 * nu)) / (alpha * alpha * tau_c0 * tau_c0);
      const real_t tStar = std::sqrt(1_r - A * lambda - B * lambda * lambda) * collisionTime_ / tau_c0;
      const real_t meff = getMeff(ac.getType(p_idx1), ac.getType(p_idx2));
      const real_t dn = 2_r * lambda * meff / tStar;
      const real_t kn = meff / std::sqrt(impactVelocityMagnitude * std::pow(tStar, 5_r));

      // calculate the normal force based on a non-linear spring-dashpot force model
      Vec3 fN = kn * std::pow(delta, 3_r/2_r) * contactNormal + dn * relVelN;
      ...
      Vec3 newTangentialSpringDisplacement = rotatedTangentialDisplacement.sqrLength() <= 0_r ? // avoid division by zero
                                             Vec3(0_r) :
      ...
      const real_t tangentialVelocityThreshold = 1e-8_r;
      ...
      else
      {
         ...
         if(stiffnessT > 0_r ) newTangentialSpringDisplacement = ( fFrictionAbs * t - dampingT * relVelT ) / stiffnessT;
