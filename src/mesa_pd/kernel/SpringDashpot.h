...
public:
                             ...
                             const real_t meff)
   {
      ...
      setStiffness(type1, type2, math::pi * math::pi * meff / ( collisionTime * collisionTime * ( 1_r - lnDryResCoeff * lnDryResCoeff / ( math::pi * math::pi + lnDryResCoeff* lnDryResCoeff ))  ));
      setDampingN( type1, type2, - 2_r * std::sqrt( meff * getStiffness(type1, type2) ) * ( lnDryResCoeff / std::sqrt( math::pi * math::pi + ( lnDryResCoeff * lnDryResCoeff ) ) ));
   }
private:
...
inline SpringDashpot::SpringDashpot(const uint_t numParticleTypes)
{
   ...
   stiffness_.resize(numParticleTypes * numParticleTypes, 0_r);
   dampingN_.resize(numParticleTypes * numParticleTypes, 0_r);
   dampingT_.resize(numParticleTypes * numParticleTypes, 0_r);
   friction_.resize(numParticleTypes * numParticleTypes, 0_r);
}
