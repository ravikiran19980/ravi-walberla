...
inline ForceLJ::ForceLJ(const uint_t numParticleTypes)
{
   ...
   epsilon.resize(numParticleTypes * numParticleTypes, 0_r);
   sigma.resize(numParticleTypes * numParticleTypes, 0_r);
}
...
inline void ForceLJ::operator()(const size_t p_idx, const size_t np_idx, Accessor& ac) const
{
   ...
   if (p_idx != np_idx)
   {
      ...
      const real_t force = 48_r * sr6 * ( sr6 - 0.5_r ) * sr2 * getEpsilon(ac.getType(p_idx), ac.getType(np_idx));
