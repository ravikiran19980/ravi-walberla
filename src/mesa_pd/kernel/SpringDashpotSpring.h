...
public:
   ...
   real_t calcCoefficientOfRestitution(const size_t type1,
                                       ...
                                       const real_t meff)
   {
      auto a = 0.5_r * getDampingN(type1, type2) / meff;
                             ...
                             const real_t meff)
   {
      ...
      setStiffnessN(type1, type2, math::pi * math::pi * meff / ( collisionTime * collisionTime * ( 1_r - lnDryResCoeff * lnDryResCoeff / ( math::pi * math::pi + lnDryResCoeff* lnDryResCoeff ))  ));
      setDampingN( type1, type2, - 2_r * std::sqrt( meff * getStiffnessN(type1, type2) ) * ( lnDryResCoeff / std::sqrt( math::pi * math::pi + ( lnDryResCoeff * lnDryResCoeff ) ) ));
   }
private:
...
inline SpringDashpotSpring::SpringDashpotSpring(const uint_t numParticleTypes)
{
   ...
   coefficientOfFriction_.resize(numParticleTypes * numParticleTypes, 0_r);
}
                                      ...
                                      const real_t dt) const
{
   ...
   if (p_idx1 != p_idx2)
   {
      ...
      auto tangentialDisplacement = Vec3(0_r);
