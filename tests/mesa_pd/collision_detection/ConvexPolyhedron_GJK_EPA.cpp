...
void runComparisonTest(Support& testSupport, Support& convPoly, Support& box) {
   ...
   if (polyCollided) {
      auto eps = 1e-3_r;
      WALBERLA_CHECK_FLOAT_EQUAL_EPSILON(polyNormal, boxNormal, 1e-2_r);
...
int main(int argc, char** argv){
   ...
   Sphere sphere_(0.5_r);
   Support sphere(Vec3(0.5_r,0.5_r,0.9_r), Rot3(), sphere_);
   runComparisonTest(sphere, convPoly, box);

   /// Fuzzy Testing

   AABB collisionTestAABB(mesa_pd::Vec3(-1_r), mesa_pd::Vec3(1_r));
   std::mt19937 rng;
   ...
   for(uint_t i = 0; i < 500; ++i) {
      ...
      Box fuzzBox_(Vec3(1_r,0.5_r,0.5_r));
