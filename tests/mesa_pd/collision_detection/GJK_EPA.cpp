...
static const std::array depths = {real_t(-1e-5), 1e-5_r, 1e-4_r, 1e-2_r, 0.1_r, 1.0_r};
static const real_t test_accuracy = 1e-3_r;
#else
static const std::array depths = {1e-2_r, 0.1_r, 1.0_r};
...
//      WALBERLA_LOG_DEVEL( normal2 << " " << pos2 << " " <<  comp_pen_depth2);
      if(depth > 0.0_r){
         ...
         checkContact( pos2, normal2, comp_pen_depth2,
                       witnesspoint + depth * witnessmove, -1.0_r*real_axis, -depth * penetration_factor, planeNormal, accuracy );
      }
      if(depth < 0.0_r){
         WALBERLA_CHECK(!result1);
         WALBERLA_CHECK(!result2);
...
void MainTest()
{
   ...
   checkContact( contactPoint,  normal, penetrationDepth,
                 Vec3(0.75_r, 0, 0), Vec3(-1.0_r, 0, 0), -0.5_r, Vec3(0,0,0) );
   ...
   Box bx_3011(Vec3(30.0_r,1,1));
   Support box2_1(Vec3(20.0_r,0,0), Rot3(), bx_3011);
   Vec3 wp2(5, 0, 0);
   Vec3 wpm2(-0.5_r,0,0);
   Vec3 axis2(-1,0,0);
   ...
   Support sphere3_2(Vec3(10.0_r,0,0), Rot3(), s_5);
   Vec3 wp3(5, 0, 0);
   Vec3 wpm3(-0.5_r,0,0);
   Vec3 axis3(-1,0,0);
   ...
   //compute rotation.
   real_t angle = walberla::math::pi/4.0_r;
   Vec3 zaxis(0, 0, 1);
   Quat q4(zaxis, angle);

   //create turned box
   real_t sqr2 = std::sqrt(2.0_r);
   Support box4_1( Vec3(5.0_r*(1.0_r+sqr2), -5.0_r, 0), Rot3(q4), bx_101010);
   Support box4_2( Vec3(0, 0, 0),                       Rot3(), bx_101010);
   Vec3 wp4(5, -5, 0);
   Vec3 wpm4(-0.25_r,real_t(+0.25),0);
   ...
   Support box5_2(Vec3(15.0_r,5.5_r, 0), Rot3(), bx_3011);
   Vec3 wp5(3.75_r, 5, 0);
   Vec3 wpm5(0, -0.5_r, 0);
   Vec3 axis5(0, -1, 0);
   ...
   Support box6_1(Vec3(5.0_r*(1.0_r+sqr6_3), 0, 0), Rot3(q6), bx_101010);
   ...
   Support box7_2(Vec3(0, 0,7.5_r), Rot3(), bx_555);
   Vec3 wpm7(0, 0, -0.5_r);
   Vec3 wp7(0, 0, 5.0_r);
...
void FuzzyEllipsoidEllipsoid(uint_t iterations)
{
   ...
   Support e2(Vec3(3.999_r,0,0), Rot3(), el);
   ...
   for (uint_t i = 0; i < iterations; ++i)
   {
      ...
      WALBERLA_CHECK_FLOAT_EQUAL(normal, Vec3(-1_r,0,0));
      WALBERLA_CHECK_FLOAT_EQUAL(contactPoint, Vec3(3.999_r * 0.5_r, 0, 0));
      WALBERLA_CHECK_FLOAT_EQUAL(penetrationDepth, 3.999_r - 4_r);
   }
}
...
void FuzzyEllipsoidBox(uint_t iterations)
{
   ...
   auto penetrationDepth     = 0_r;

   auto gjk_normal           = Vec3();
   auto gjk_contactPoint     = Vec3();
   auto gjk_penetrationDepth = 0_r;

   Box       bx(Vec3(2,2,2));
   Ellipsoid el(Vec3(2,2,2));
   Support   p1(Vec3(0,0,0), Rot3(), el);
   Support   p2(Vec3(3.999_r,0,0), Rot3(), bx);
   ...
   for (uint_t i = 0; i < iterations; ++i)
   {
      ...
      while (!bCollision)
      {
         p2.pos_ -= Vec3(0.1_r,0,0);
         if (p2.pos_[0]<0_r) WALBERLA_ABORT("ERROR");
      ...
      //WALBERLA_CHECK_FLOAT_EQUAL_EPSILON(gjk_normal, normal, 1e-2_r);
      //WALBERLA_CHECK_FLOAT_EQUAL_EPSILON(gjk_contactPoint, contactPoint, 1e-2_r);
      //WALBERLA_CHECK_FLOAT_EQUAL_EPSILON(gjk_penetrationDepth, penetrationDepth, 1e-2_r);
   }
}
