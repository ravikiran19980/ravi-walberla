...
int main(int argc, char** argv) {
   ...
   /// BlockForest
   Vector3< real_t > minCorner(0_r, 0_r, 0_r);
   Vector3< real_t > maxCorner(4_r, 4_r, 4_r);
   ...
   /// Mesh Particles
   math::Rot3< real_t > cubeRotation(Vector3< real_t >(static_cast< real_t >(M_PI) / 4_r, 0_r, 0_r));
   for (uint_t x = 0; x <= 1; ++x) {
      for (uint_t y = 0; y <= 1; ++y) {
         for (uint_t z = 0; z <= 1; ++z) {
            // position of new mesh cube slightly shifted inwards
            Vector3< real_t > pos((0.05_r + static_cast< real_t >(x) * maxCorner[0] - static_cast< real_t >(x) * 0.1_r),
                                  0.05_r + static_cast< real_t >(y) * maxCorner[1] - static_cast< real_t >(y) * 0.1_r,
                                  0.05_r + static_cast< real_t >(z) * maxCorner[2] - static_cast< real_t >(z) * 0.1_r);
