...
int main( int argc, char ** argv )
{
   ...
   auto  smallSphere = ss->create<data::Sphere>( 1.2_r );

   ss->shapes[smallSphere]->updateMassAndInertia(1_r);
   ...
   {
      auto p                       = ps->create();
      p->getPositionRef()          = Vec3(static_cast< real_t >(i), 0_r, 0_r);
   ...
   cs.forEachContact(false, kernel::SelectAll(), cs, [&normal, &dist, &contactCount](size_t idx, data::ContactStorage &css){
      ...
      WALBERLA_CHECK_FLOAT_EQUAL(css.getPosition(idx), Vec3(real_t(2*idx+1), 0_r, 0_r));
