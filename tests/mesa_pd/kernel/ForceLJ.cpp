...
int main( int argc, char ** argv )
{
   ...
   //init kernels
   kernel::ForceLJ lj(1);
   lj.setEpsilon(0, 0, 0.121_r);
   lj.setSigma  (0, 0, 0.212_r);

   //check equilibrium distance
   const real_t eq_dist = std::pow(2_r, 1.0_r/6.0_r);
   ...
   accessor.setPosition(1, Vec3( eq_dist * lj.getSigma(0, 0) * 100_r, 0, 0));
   lj(0, 1, accessor);
   ...
   p1.getForceRef()           = Vec3(0,0,0);
   p2.getForceRef()           = Vec3(0,0,0);
   p2.getPositionRef()        = Vec3( 2_r * lj.getSigma(0, 0), 0, 0);
   lj(0, 1, accessor);
   WALBERLA_CHECK_GREATER( p1.getForce()[0], 0_r, p1 << p2 );
   WALBERLA_CHECK_LESS   ( p2.getForce()[0], 0_r, p1 << p2 );
   ...
   lj(0, 1, accessor);
   WALBERLA_CHECK_LESS   ( p1.getForce()[0], 0_r, p1 << p2 );
   WALBERLA_CHECK_GREATER( p2.getForce()[0], 0_r, p1 << p2 );

   //action = reaction
   p1.getForceRef()           = Vec3(0,0,0);
   p2.getForceRef()           = Vec3(0,0,0);
   p2.getPositionRef()        = Vec3( 1, 2, 3) * 0.1_r;
   lj(0, 1, accessor);
