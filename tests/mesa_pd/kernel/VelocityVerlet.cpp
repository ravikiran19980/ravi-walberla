...
public:
   ParticleAccessorWithShape(std::shared_ptr<data::ParticleStorage>& ps)
      : ParticleAccessor(ps)
   {
      sp.updateMassAndInertia(1234_r);
   }
...
private:
   data::Sphere sp = data::Sphere(0.6_r);
};
...
int main()
{
   ...
   const Vec3 startingVelocity(1_r,2_r,3_r);
   ...
   const real_t dt = 0.001_r;

   const real_t k = 10000_r;
   ...
   p.getLinearVelocityRef()  = analyticVelocity(0_r);
   p.getForceRef()           = - k * p.getPosition();

   p.getAngularVelocityRef() = analyticAngVel(0_r);
   ...
   WALBERLA_CHECK_FLOAT_EQUAL_EPSILON(p.getPosition(), Vec3(0), 1e-2_r);
   WALBERLA_CHECK_FLOAT_EQUAL_EPSILON(p.getLinearVelocity(), startingVelocity, 1e-2_r);
