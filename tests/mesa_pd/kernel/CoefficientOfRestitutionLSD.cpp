...
int main( int argc, char** argv )
{
   ...
   real_t densitySphere = 1.5_r;
   real_t collisionTime = 10_r;
   ...
   const real_t dampingN = - 2_r * std::sqrt( Mij * stiffnessN ) * ( lnDryResCoeff / std::sqrt( math::pi * math::pi + ( lnDryResCoeff * lnDryResCoeff ) ) );
   ...
   {
      WALBERLA_CHECK_LESS(relativeError, 0.01_r, "Error in simulated coefficient of restitution too large: " << simulatedCoefficientOfRestitution << " vs ref " << restitutionCoeff);
   }
   else
   {
      WALBERLA_CHECK_LESS(relativeError, 0.03_r, "Error in simulated coefficient of restitution too large: " << simulatedCoefficientOfRestitution << " vs ref " << restitutionCoeff);
   }
