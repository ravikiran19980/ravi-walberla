...
std::vector<Vector3<real_t>> generatePointCloudIcosahedron()
{
   ...
   for( auto one : {-1_r, 1_r} )
      ...
      {
         points.emplace_back( 0_r, static_cast< real_t >(one), static_cast< real_t >(phi) );
         points.emplace_back( static_cast< real_t >(one), static_cast< real_t >(phi), 0_r );
         points.emplace_back( static_cast< real_t >(phi), 0_r, static_cast< real_t >(one) );
      }
...
std::vector<Vector3<real_t>> generatePointCloudDodecahedron()
{
   ...
   for( auto phi : {-PHI, PHI} )
      ...
      {
         points.emplace_back( 0_r, static_cast< real_t >(piv), static_cast< real_t >(phi) );
         points.emplace_back( static_cast< real_t >(piv), static_cast< real_t >(phi), 0_r );
         points.emplace_back( static_cast< real_t >(phi), 0_r, static_cast< real_t >(piv) );
      }
...
std::vector<Vector3<real_t>> generatPointCloudOnSphere( const real_t radius, const uint_t numPoints )
{
   ...
   for( auto & p : pointCloud )
   {
      real_t theta = 2 * real_c(math::pi) * distribution(rng);
